...
The design goal is simple: you do the implementation work, while Codex behaves like a strong Rust coach who preserves context, reviews your decisions, and keeps your learning trajectory visible over time.
...
- startup prompts rebuild context after restarts
...
- `state/`: persistent context that survives restarts
- `ops/`: operating guides for the coaching workflow

## Recommended Use

1. Read [learning-path.md](./learning-path.md).
2. Set the active session in `state/current-session.md`.
